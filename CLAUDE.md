...
- `WebhookAction` — triggers outbound webhooks. `Webhook.init()` (called on every API startup/reload) only auto-`execute()`s keys for a `checkOnStart` webhook — that's the only case with no owning function to ever otherwise check/add it or re-arm its periodic TTL recheck (`WebhookKey.schedule()`, run at the end of a successful check). A `WebhookAction`-created key (`checkOnStart == false`) is restored into memory (so `getWebhookKeyOrCreate()` can find it) but deliberately **not** executed at startup — its owning function is what checks/re-verifies it, on that function's own schedule, whenever it next runs; the persisted key/id/ttl is available immediately, nothing acts on it until then. This is deliberate, not an oversight: auto-executing it here too would race that function's own invocation (both would try to check/add the same key at startup) for no benefit, since this generic pass has no way to supply whatever custom variables the function's `WebhookAction` would. Every automatic invocation (`execute()`'s no-arg form and `schedule()`'s recurring TTL recheck) uses the `WebhookKey` itself as the store, i.e. `{{var::webhook.xxx}}` — only a `WebhookAction`'s own direct invocation uses a `"handler"`-named store layering its `variables:` over the key's fixed fields (`{{var::handler.xxx}}`, e.g. Rachio's `check`/`add`), and that convention isn't preserved on a later automatic recheck of the same key
...
There is no PersistentRecord/DB table for API config anymore. Config and credential-ish runtime state live in the `APIResource` record (`records/APIResource.java`), embedded as one JSON blob per named API resource:
...
Webhook keys are **not** part of `APIResource` — see below for where they actually live.

**Important gotcha:** because variables/certificate live inside the *same* resource as the YAML config, persisting a runtime change (e.g. a refreshed OAuth2 token) goes through `API.persistResource(...)` → `APIManager.updateResource(...)` → `NamedResourceHandler.modify(...)`, which pushes a real resource change through ConfigurationManager. That **triggers a full async reload of that API instance** (`APIResourceHandler.onResourceUpdated` tears down and reconstructs it), the same as if someone edited the YAML by hand. `Variables.setVariable(String, Object)`/`clearVariable(...)` (the runtime entry points used by auth flows and the OAuth2 callback) persist; the config-parse-time overloads used by `parse()`/`initializeVariables()` (which run on *every* startup/reload) deliberately do **not**, to avoid a reload loop. Keep that split when touching this code — routing a startup-time call through the persisting path is an easy way to create an infinite reload loop.

**Webhook keys are persisted entirely outside ConfigurationManager**, via `api/webhooks/WebhookKeyStore.java`, so creating/updating one doesn't force a full API reload — the API is typically already running mid-execution when a webhook key changes, and reloading it is disruptive for no benefit (a webhook key isn't config, it's runtime state tracked against an external system). `WebhookKeyStore` reads/writes one JSON file per webhook (`<gatewayDataDir>/modules/com.kyvislabs.api.client/<apiName>/webhooks/<webhookName>.json`, via `GatewayHook.GSON`), the same pattern `StoreFileAction` uses for saved files. Both `Webhook.java` (the normal runtime path) and `APIMigrationStrategy.java` (which seeds this store directly for an 8.1→8.3 migration, grouping legacy `APIWebhook` rows by webhook name) go through this same shared class — there is no `APIResource.webhookKeys` field for either to fall back to; it was removed once the file-based store existed everywhere it needed to.
